  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        // FIX 3: Robust fallback logic for slug creation if not provided by database
      ...
      <main className="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto max-w-7xl mx-auto w-full">
        ...
        {successMsg && (
          <div className="bg-[#E8F5E9] text-[#2E7D32] text-sm border border-[#C8E6C9] px-4 py-3 rounded-xl flex justify-between items-center shadow-sm">
            ...
            <button onClick={() => setSuccessMsg("")} className="text-emerald-800 text-lg font-bold" type="button">×</button>
          </div>
        )}
        {error && (
          <div className="bg-[#FFEBEE] text-[#C62828] text-sm border border-[#FFCDD2] px-4 py-3 rounded-xl flex justify-between items-center shadow-sm">
            ...
            <button onClick={() => setError("")} className="text-red-800 text-lg font-bold" type="button">×</button>
          </div>
        )}
          ...
          <div className="lg:col-span-5 bg-[#FAF7F2] border border-[#E8E2D6] p-6 rounded-2xl shadow-sm">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="pt-2 flex gap-2">
                ...
                {editingId && (
                  <button type="button" onClick={clearForm} className="bg-[#EFEBE9] text-[#5D4037] py-2.5 px-4 rounded-xl text-sm">Cancel</button>
                )}
              </div>
            </form>
          </div>
                        ...
                        <tr key={tId} className="hover:bg-[#FAF7F2]/50 transition-colors">
                          ...
                          <td className="p-3.5">
                            <div className="flex gap-2 justify-center items-center">
                              <button onClick={() => handleEditTrigger(temple)} className="border border-stone-200 bg-white px-2.5 py-1 rounded-lg font-bold">✏️ Edit</button>
                              <button onClick={() => handleDeleteAction(tId)} className="border border-red-100 bg-[#FFEBEE] text-[#C62828] px-2.5 py-1 rounded-lg font-bold">🗑️ Delete</button>
                            </div>
                          </td>
                        </tr>
                      );
